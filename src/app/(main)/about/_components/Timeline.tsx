      ...
      <div className="relative my-auto flex h-[600px] items-center justify-center md:ml-6 md:flex-[10]">
        <div className="grid h-full w-full grid-rows-[auto_auto_1fr] items-stretch justify-items-start gap-5 rounded-xl bg-white pb-1 pt-4">
          <div className="mt-2 flex w-full items-center justify-between gap-2 md:mt-0 md:justify-start">
            <MdKeyboardArrowUp
              ...
              className={`h-10 w-10 -rotate-90 cursor-pointer rounded-full p-2 transition-all md:rotate-0 ${
                selectedIndex > 0
                  ? "scale-100 bg-gray-200 hover:bg-primary_lightest hover:text-primary"
                  : "bg-gray-100 text-gray-500"
              }`}
            />
            <p className="flex flex-col items-center text-base font-bold md:order-3 md:flex-row md:gap-[0.375rem] md:text-xl">
              <span className="Bebas text-3xl font-normal leading-none text-primary md:font-Nunito md:text-xl md:font-extrabold">
          ...
          <div className="flex-col gap-5">
            ...
            <p className="text-neutral-700">{currentDescription}</p>
          </div>
          <div className="flex overflow-hidden rounded-[.75rem]">
            <img
              src={currentImage}
              alt="Science Club Event"
  ...
  return (
    ...
    >
      <div className="h-[4.5rem] w-16">
        ...
        <p
          className={`transition-all ${
            current ? "text-base font-bold text-neutral-800" : "scale-85 text-sm text-gray-400"
          }`}
        >
          {time2}
        </p>
      ...
      <div
        className={`absolute left-[50%] top-[4.25rem] z-10 hidden h-5 w-5 -translate-x-[50%] items-center justify-center rounded-full bg-primary shadow-[5px_5px_20px_10px_#00000024] transition-all duration-300 md:left-[4rem] md:top-[50%] md:flex md:-translate-y-[50%] md:translate-x-0 ${
