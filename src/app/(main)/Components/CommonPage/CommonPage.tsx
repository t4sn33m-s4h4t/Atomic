      ...
      {sections.map(({ heading, content, subSections }, index) => {
        return (
          <React.Fragment key={index}>
            <div className="section" id={subNum + index.toString()}>
              <h2
                className={
...
const TOCList = ({
  ...
  return (
    ...
    >
      {sections.map(({ heading, subSections }, index) => {
        return (
          <li key={index} className="list-disc cursor-pointer ml-[0.9rem]">
            <a href={'#' + subNum + index}>
              <p
                className={
                  'text-base hover:underline' + (subNum ? ' font-light' : ' font-medium')
                }
              >
                {heading}
              </p>
  ...
  return (
    <div className="mt-[81px] bg-[#F6F6F6]">
      <div
        ...
        className="fixed top-[80px] left-0 h-1 z-30 bg-[#486581] transition ease-linear w-screen"
      ></div>
      <div className="container pt-5 pb-16">
        <div className="flex gap-5 items-center sm:justify-start">
          ...
          <h1 className="text-6xl">
            <span className="text-[#486581]">{firstWord}</span>
            <span>{restWord}</span>
          </h1>
        </div>
          ...
          {hasTableOfContent ? (
            <div className="bg-white shadow-2xl rounded-xl p-8 basis-[310px] md:sticky top-[96px] h-[65vh]">
              <h2 className="text-3xl">
                <span className="text-[#486581]">Table</span> of contents
              </h2>
              <div className="mt-4">
